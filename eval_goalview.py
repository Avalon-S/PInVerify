...
def evaluate(config_env, args, num_episodes: Optional[int] = None) -> Dict[str, float]:
    ...
    # optional pre-filter of the episode list (--filter-episodes)
    if args.filter_episodes:
        print(f"Loading filter list from: {args.filter_episodes}")
        try:
            with open(args.filter_episodes, "r", encoding="utf-8") as f:
                filter_data = json.load(f)
            # handle the nested form
            if isinstance(filter_data, dict) and "episodes" in filter_data:
                filter_list = filter_data["episodes"]
            else:
                filter_list = filter_data
                
            # lookup set of (scene_id basename, episode index)
            # Habitat's runtime episode_id is a load-order index, not the id in the source JSON
            allowed_indices = set()
            for item in filter_list:
                sid = os.path.basename(os.path.dirname(item["scene_id"]))  # parent directory name
                eid = str(item["episode_id"])  # the runtime index
                allowed_indices.add((sid, eid))
            print(f"Filter loaded. {len(allowed_indices)} episodes to run.")
            
            # filter henv.episodes
            # ids in bad_episodes.json are per-scene indices starting at 0,
            # so match them after grouping by scene
            
            original_episodes = henv.episodes
            filtered_episodes = []
            
            # group the original episodes by scene
            from collections import OrderedDict
            scene_episodes_map = OrderedDict()  # scene_key -> list of (original_idx, ep)
            for idx, ep in enumerate(original_episodes):
                ep_scene_key = os.path.basename(os.path.dirname(ep.scene_id))
                if ep_scene_key not in scene_episodes_map:
                    scene_episodes_map[ep_scene_key] = []
                scene_episodes_map[ep_scene_key].append((idx, ep))
            
            # match per scene by index
            for scene_key, eps_list in scene_episodes_map.items():
                for local_idx, (original_idx, ep) in enumerate(eps_list):
                    local_idx_str = str(local_idx)
                    if (scene_key, local_idx_str) in allowed_indices:
                        filtered_episodes.append(ep)
            
            print(f"Pre-filtered: {len(original_episodes)} -> {len(filtered_episodes)} episodes")
            henv._dataset.episodes = filtered_episodes
            henv.episodes = filtered_episodes
    ...
    with tqdm(total=num_episodes, desc=f"Worker {args.job_index}/{args.num_jobs}") as pbar:
        while count_episodes < num_episodes:
            ...
            current_ep = env.current_episode
            
            # filtering already happened above, no runtime skip needed
...
def parse_args():
    ...
    # optional episode filter, for re-checking repaired episodes
    parser.add_argument(
        "--filter-episodes", 
        type=str, 
        default=None,
        help="Path to json file containing list of episodes to run (e.g. bad_episodes.json). If provided, only episodes in this list will be evaluated."
    )
