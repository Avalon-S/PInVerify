#!/usr/bin/env python3
"""
Generate SFT training data with dual-image input + CoT reasoning.
...
v2 changes:
- Removed navigation field from <answer> (only verification + action)
- CoT [Navigation] uses unvisited available sectors (not all navigable)
- Multi-step positive exploration with increasing stop probability
- Trap view recovery training (20% of positive pairs start at trap view)
- Step history for multi-step context
- Visibility warning for trap views
- neg_same 50% multi-step (up from 30%)
...
Usage:
    python training/prepare_sft_data.py \
        ...
        --attr-cache ./data/pv_dataset/train_sft/attr_cache_train.json \
...
def build_cot_neg_same(attrs, available_dirs, mismatch_attr):
