...
class Env:
    ...
    def reset(self) -> Observations:
        ...
        if hasattr(self._dataset, "category_to_task_category_id"):
            ...
            # Load templates only on the first reset, on a scene change, or after they are cleared.
            # Reloading on every reset accumulates duplicate handles with version suffixes.
            _current_scene = getattr(self._current_episode, "scene_id", None)
            _need_load = (
                not hasattr(self, "_pin_templates_loaded_scene")
                or self._pin_templates_loaded_scene != _current_scene
                or obj_templates_mgr.get_num_templates() == 0
            )
            if _need_load:
                if os.path.isdir(obj_path):
                    obj_templates_mgr.load_configs(obj_path, True)
                self._pin_templates_loaded_scene = _current_scene
